...
private:
	...
	node *tree;
	int search(node *root, int level,int x){
		if(root==NULL)
			return 0;
		bool f = x&(1<<level);
		if(root->child[f^1]!=NULL)
			return (1<<level)+search(root->child[f^1],level-1,x);
...
int main(){
	...
	while(t--){
		...
		cin>>n;
		Trie T;
		int temp,x = 0,result = 0;
		T.insert(0);
		for (int i = 0; i < n; ++i)
		{
			cin>>temp;
			x^=temp;
			result = max(result,T.query(x));
			T.insert(x);
		}
		cout<<result<<endl;
	}
